...
@section("content")
    ...
    <section id="archives-write" class="board-wrap">
        ...
        <form action="/admin/qna/update_proc" method="post" name="register_form" enctype="multipart/form-data" >
            @csrf
            <div class="table-wrap table02">
                <table class="write-table">
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <input type="text" name="subject" id="tbl-subject" class="tbl-input w100p" value="{{ $lists->subject }}" required />
                        </td>
                    </tr>
                    <tr>
{{--                        <th>--}}
{{--                            <label for="tbl-contents" class="required_mark">내용</label>--}}
{{--                        </th>--}}
...
{{--                            <input type="file" id="attachments1" name="photo1" class="file-hidden2">--}}
                            <input type="file" id="attachments1" name="photo1" class="file-hidden" value="{{ $lists->photo1 }}">
                            <input type="hidden" id="attachments1" name="ori_photo1" class="file-hidden" value="{{ $lists->photo1 }}">
                            <input class="file-name tbl-file w300" name="file1name" value="{{ $lists->ori_photo1 }}" placeholder="선택된 파일이 없습니다." >
...
{{--                            <label class="file-label">찾아보기<input class="file-02 file-hidden" type="file" name="photo2" onchange="javascript:document.getElementById('attachments2').value=$(this).val().split('/').pop().split('\\').pop();"></label>--}}
                            <input type="file" id="attachments2" name="photo2" class="file-hidden" value="{{ $lists->ori_photo2 }}" >
                            <input class="file-name tbl-file w300" name="file2name" value="{{ $lists->ori_photo2 }}" placeholder="선택된 파일이 없습니다." >
                ...
                <div class="btn-wrap">
                    <a href="/admin/qna" class="btn01 btn_cancel">취소</a>
                    <button type="submit" name="send" class="confirm-link btn01 btn_submit">저장</button>
                </div>
